...
//    }
    ...
    [_avatarButton setImageWithURL:[NSURL URLWithString:avatarUrlString] placeholderImage:IMG_NOPIC AndSaleSize:CGSizeMake(_avatarButton.frame.size.width*2, _avatarButton.frame.size.height*2)  ] ;
}
...
- (void)_setup
{
    ...
    [_bannerView addSubview:_bannerImageView];
    ...
    CGFloat waterDropRefreshHeight = self.frame.size.height - _showUserInfoViewOffsetHeight ;
...
//    [self addSubview:self.waterDropRefresh];
    ...
    CGFloat avatarButtonHeight = 70;
    
    self.showUserInfoViewOffsetHeight = 10.0f ;//CGRectGetHeight(self.frame) - waterDropRefreshHeight / 3 - avatarButtonHeight;
    ...
    _avatarButton = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, avatarButtonHeight, avatarButtonHeight)];
    _avatarButton.center = CGPointMake(APPFRAME.size.width / 2, self.frame.size.height / 3) ;
    
    _avatarButton.userInteractionEnabled = YES ;
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(pressedHeadPic)] ;
    [_avatarButton addGestureRecognizer:tap] ;
    [TeaCornerView setRoundHeadPicWithView:_avatarButton] ;
    
    //    [_avatarButton addTarget:self action:@selector(pressedHeadPic) forControlEvents:UIControlEventTouchUpInside] ;
    
    
    _userNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 300, 34)];
    _userNameLabel.center = CGPointMake(_avatarButton.center.x, _avatarButton.center.y + 60) ;
    _userNameLabel.textColor = [UIColor whiteColor];
    _userNameLabel.backgroundColor = [UIColor clearColor];
    _userNameLabel.shadowColor = [UIColor blackColor];
    _userNameLabel.shadowOffset = CGSizeMake(0, 2);
    _userNameLabel.font = [UIFont boldSystemFontOfSize:20.0f];
...
//    _birthdayLabel.font = [UIFont systemFontOfSize:14.0f];
    ...
    [button addTarget:self action:@selector(clickShowView) forControlEvents:UIControlEventTouchUpInside] ;
    [button setFrame:CGRectMake(0, 0, _showView.frame.size.width, _showView.frame.size.height)] ;
...
- (void)pressedHeadPic
{
    NSLog(@"pressedHeadPic") ;
    [self.delegate pressedUserHeadPicture] ;
}

- (void)clickShowView
{
    NSLog(@"clickShowView") ;
    [self.delegate pressedUserInfo] ;
}
