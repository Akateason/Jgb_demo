...
- (void)drawRect:(CGRect)rect
{
    ...
    [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
        ...
        CGFloat y = ((self.height - self.selectionIndicatorHeight) / 2) + (self.selectionIndicatorHeight - stringHeight / 2) - 3.0;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        if (segment != self.selectedIndex) { // click other
            [self setSelectedIndex:segment animated:YES];
        }else { // click self
            // add teason 20140828 start
            [self.delegate sendCurrentIndexEveryPressed:segment];
            // add teason 20140828 e n d
        }
    }
//    NSLog(@"tap !") ;
}
