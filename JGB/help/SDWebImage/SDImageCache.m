...
//    NSLog(@"key : %@",key);
//    const
    __strong const char *str = [key UTF8String];
...
- (void)removeImageForKey:(NSString *)key
{
    ...
    NSString *strPath = [self cachePathForKey:key] ;
    NSLog(@"removeImageForKey : %@",[self cachePathForKey:key]) ;

    [memCache removeObjectForKey:key];

    NSError *error = nil;
    
    BOOL exist = [[NSFileManager defaultManager] fileExistsAtPath:strPath]  ;
    NSLog(@"exist : %d",exist) ;
    
    BOOL b = [[NSFileManager defaultManager] removeItemAtPath:strPath error:&error];
    NSLog(@"del : %d",b) ;
    if (error != nil) {
        NSLog(@"error : %@",error) ;
    }
