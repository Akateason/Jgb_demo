...
- (void)_init
{
    ...
    self.pageControl.pageIndicatorTintColor = [UIColor colorWithWhite:0.7 alpha:0.5];
...
- (void)reloadData
{
    for (UIView *aView in self.scrollView.subviews) {
        if ([aView isKindOfClass:[UIImageView class]]) {
            ...
            int tagCurrent = imageView.tag ;
            int indexCurrent = tagCurrent - kStartTag ;
            [self.imagePlayerViewDelegate imagePlayerView:self loadImageForImageView:imageView index:indexCurrent];
        }
    }
}
...
- (void)initWithCount:(NSInteger)count delegate:(id<ImagePlayerViewDelegate>)delegate edgeInsets:(UIEdgeInsets)edgeInsets
{
    ...
    [self.scrollView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|-0-[%@]-0-|", [imageViewNames firstObject]]
                                                                            options:kNilOptions
                                                                            metrics:nil
...
- (void)handleTapGesture:(UIGestureRecognizer *)tapGesture
{
    ...
    if (self.imagePlayerViewDelegate && [self.imagePlayerViewDelegate respondsToSelector:@selector(imagePlayerView:didTapAtIndex:AndLR:)]) {
        [self.imagePlayerViewDelegate imagePlayerView:self didTapAtIndex:index AndLR:lr];
    }
}
